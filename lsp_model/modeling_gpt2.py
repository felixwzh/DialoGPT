...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, persona_ids,position_ids=None, token_type_ids=None, lm_labels=None, past=None):        
        ...
        if lm_labels is not None:
            ...
            ppl = torch.exp(torch.mean(torch.sum(loss1, dim=1).float()
                                       / label_size.float()))
            loss_sum=torch.sum(loss1).float()
            labal_sum=torch.sum(label_size).float()
            ...
            # print('loss')
            # print(loss)
            # print('torch.sum(loss1, dim=1).float()')
            # print(torch.sum(loss1, dim=1).float())
            # print('torch.sum(loss1, dim=1).float()/ label_size.float()')
            # print(torch.sum(loss1, dim=1).float()/ label_size.float())
            # print('ppl')
            # print(ppl)
            # print(labal_sum)
            ...
            return loss, ppl, loss_sum, labal_sum
    ...
    def forward_origin(self, input_ids,persona_ids, position_ids=None, token_type_ids=None, lm_labels=None, past=None):
        hidden_states, presents = self.transformer(input_ids,persona_ids, position_ids, token_type_ids, past)
        lm_logits = self.lm_head(hidden_states)
        if lm_labels is not None:
    ...
    def prepare_inputs_for_generation(self, input_ids, past, **kwargs):
        # TODO: modify this
        # FIXME: refer to the up-to-date modeling_gpt.py 
    ...
    def generate(
        self,
        input_ids=None,
        max_length=None, # FIXME: how to set max_length?
        min_length=None, # 0
        do_sample=None, # TODO: we can choose either False or True, to test the results.
        early_stopping=None, 
        num_beams=None,
        temperature=None,
        top_k=None,
        ...
        pad_token_id=None, # 0
        eos_token_id=None, # END_OF_TEXT_TOKEN = '<|endoftext|>'
        length_penalty=None, # 1
        no_repeat_ngram_size=None, # leave it
        num_return_sequences=None, # 1
        attention_mask=None, # TODO: figure out this
        decoder_start_token_id=None, # not related.
        use_cache=None, # True
    ):# TODO: add persona_ids in this function. 
        ...
        #  we should set the attention_mask by our selfs.
        assert attention_mask is not None, "set attention_mask yourself!"
        ...
        """
        # GPT2 is not encoder_decoder, so we ignore this part here.
        # TODO: maybe we should first run this and see if this is ignored by the self.config.is_encoder_decoder value.
        if self.config.is_encoder_decoder:
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            # TODO: 1. learn this ** and * param in python,
            # TODO: 2 . modify this line for safe.

            next_token_logits = outputs[0][:, -1, :]

            # if model has past, then set the past variable to speed up decoding
            if self._use_cache(outputs, use_cache): # TODO: what does this mean?
                past = outputs[1]
            ...
            if unfinished_sents.max() == 0: # TODO: check and see what to do with the eos token. 
                break

            # extend attention_mask for new generated input if only decoder
            # TODO: we need this, but not sure if GPT2 is encoder_decoder or not. check later.
